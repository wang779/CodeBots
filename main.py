...
class NMT:
    ...
    def train(self):
        ...
        for epoch in range(200):
            ...
            if epoch % 2 == 0 and epoch > 0:
                score = self.translate(epoch=epoch)
                self.model.save_checkpoint(
                    os.path.join("examples/pytorch/nmt/save", opt["name"]), "model_" + str(epoch) + ".pth"
                )
            if epoch >= 100 and self._stop_condition(epoch):
                break
        return max_score
    ...
    def translate(self, epoch):
        ...
        f1 = open("output/" + "/ref2_file" + str(epoch), "w", encoding="utf-8")
        f2 = open("output/" + "/predict2_file" + str(epoch), "w", encoding="utf-8")
        for data in dataloader:
            ...
            if self.use_copy:
                oov_dict = prepare_ext_vocab(
                    batch_graph=batch_graph, vocab=self.vocab, device=self.device
                )
                ref_dict = oov_dict
            else:
                oov_dict = None
            ...
            pred = self.model.translate(
                batch_graph=batch_graph, oov_dict=oov_dict, beam_size=1, topk=1
            )

            # pred_ids = pred[:, 0, :]  # we just use the top-1
            pred_ids = pred.reshape(pred.shape[0] * pred.shape[1], pred.shape[2])  # we just use the top-1

            pred_str = wordid2str(pred_ids.detach().cpu(), ref_dict)

            for i in range(pred.shape[0] * pred.shape[1]):
                f2.write(pred_str[i] + "\n")
